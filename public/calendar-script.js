...
addEventFrom.addEventListener('input' , (e) => {
    ...
    // if 2 numbers are entered, auto add:
...
function addListner() {
    ...
    days.forEach((day) => {
        day.addEventListener('click' , (e) => {
            // this is to set current day as active day
            activeDay = Number(e.target.innerHTML);

            // call active day after clicking
...
// let us show active day events and date at top
...
addEventSubmit.addEventListener('click', () => {
    ...
    if(eventsArr.length > 0) {
        // check if current day has already have an event, then add to that
    ...
    if(e.target.classList.contains('event')) {
        ...
        eventsArr.forEach((event) => {
            if (
                ...
                // if no event remained on that day remove complete day
                if (event.events.length === 0) {
                    eventsArr.splice(eventsArr.indexOf(event), 1);
                    // after removal, remove the active class of the day
